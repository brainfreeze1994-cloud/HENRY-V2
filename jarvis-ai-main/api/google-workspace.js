...
const handler = async function(req, res) {
  ...
  if (origin) {
    try {
      if (new URL(origin).host !== req.headers.host) return res.status(403).json({ error: 'Cross-origin requests are not allowed' });
  ...
  const validTypes = new Set(['docs', 'sheets', 'slides']);
  const type = validTypes.has(body.type) ? body.type : 'docs';
  const title = String(body.title || 'HENRY Document').slice(0, 120);
