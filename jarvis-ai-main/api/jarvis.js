...
const handler = async function(req, res) {
  ...
  if (!isSameOrigin(req)) return res.status(403).json({ error: 'Cross-origin requests are not allowed' });
  if (!allowRequest(req)) return res.status(429).json({ error: 'Too many requests. Please wait a minute.' });
  ...
  messages = messages.slice(-12).map(m => ({
    ...
    text: String(m?.text || m?.content || '').slice(0, 8_000)
  }));
  if (imageBase64 && (typeof imageBase64 !== 'string' || imageBase64.length > 7_000_000)) {
    return res.status(413).json({ error: 'Image is missing or too large (maximum 5 MB).' });
  }
  ...
  try {
    ...
    if (enableChainThinking || /solve|prove|calculate|derive|step by step|explain how|work out|analyze|reason through|think through/i.test(lastMsg)) {
      ...
      const conv = buildConvMessages([...messages.slice(-4), {
        role:'user',
        text: lastMsg + '\n\n[DETAILED MODE: Give a concise, verifiable explanation with numbered steps where useful. Do not reveal private reasoning.]'
      }], sys, 8);
      return res.status(200).json(parseResponse(await callLLM(GROQ_KEY, ACCOUNT_ID, API_TOKEN, conv)));
    }
